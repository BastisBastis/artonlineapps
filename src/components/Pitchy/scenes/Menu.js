...
export default class Menu extends Phaser.Scene {
  ...
  create(data) {
    ...
    this.bird=this.add.sprite(-75,cam.centerY,"bird",0)
    this.bird.setScale(0.75).setAlpha(0.5)
    this.anims.create({
      key:"fly",
  ...
  showMain() {
    ...
    this.addLabel(cam.centerX,cam.height/4,"Pitchy Bird",120).setShadow(8, 8, "#454545", 1, false, true)//idth(cam.width);
    
    this.addLabel(cam.centerX,cam.centerY,"Spela",75).setInteractive().on("pointerdown",()=>this.start(this.options))
    
    this.addLabel(cam.centerX,cam.height*0.75,"Inställningar",75).setInteractive().on("pointerdown",()=>this.showSettings())
  }
  ...
  showSettings() {
    ...
    this.addLabel(cam.centerX,cam.height/6,"Känslighet:",75);
    
    const sensitivityLabel=this.addLabel(cam.centerX,cam.height*0.7,"",75);
    ...
    const indicator=this.add.circle(cam.centerX,cam.height*0.35,20,0xff0000)
    ...
    this.addLabel(cam.centerX,cam.height*0.9,"Tillbaka",60).setInteractive().on("pointerdown",()=>{
  ...
  update(time,delta) {
    this.bird.x+=delta/12;
    if (this.bird.x>this.cameras.main.width+75)
      this.resetBird()
  }
}